...
def preprocessing(dataset, features, target, test_size, missing, transformation, seed, log_file):
    print("----- Iniciando o pre-processamento -----")
    save_log("----- Iniciando o pre-processamento -----", log_file)
    ### Pre-processing
    features_ = features
    ...
    X_test_min = dataset_minoritario.sample(quantidade_test)
    X_train_min = dataset_minoritario.drop(X_test_min.index)

    #X_test_maj = dataset_majoritario.head(quantidade_test)
    X_test_maj = dataset_majoritario.sample(quantidade_test)
    ...
    if transformation is not None:
        ...
        save_log("------- Aplicando a transformação dos dados -------", log_file)
        if isinstance(transformation, dict):
            ...
            if 'one-hot-encoding' in transformation:
                ...
                X_train, categories = one_hot_encoding(X_train, transformation['one-hot-encoding'])
                X_test, _ = one_hot_encoding(X_test, transformation['one-hot-encoding'], categories=categories)
        else:
    ...
    save_log("----- Finalizando o pre-processamento -----", log_file)
    print("\n" * 20)

    return status_missing, status_transformation, X_train, X_test, y_train, y_test

def check_balancing(X_train, X_test, y_train, y_test, balancing, hybrid_size=2, seed=42, log_file=''):

    print("----- Iniciando o Balanceamento -----")
    save_log("----- Iniciando o Balanceamento -----", log_file)
    ...
    save_log("----- Finalizando o Balanceamento -----", log_file)
    print("\n" * 20)
    print_target(y_train, y_test)
    return True, X_train, X_test, y_train, y_test


def thread_func(model_name, X_train, y_train, model_classes, lista_de_features, scoring, cv, log_file):
    print(f"Iniciando o {model_name}")
    save_log(f"Iniciando o {model_name}", log_file)
    ...
    save_log(f"Finalizando o {model_name}", log_file)

def check_feature_selection(X_train, y_train, feature_selection, feature_selection_models, models, scoring, cv, n_jobs=-1, use_threading=False, log_file=''):
    
    status_feature_selection = False
    lista_de_features = {}
    if feature_selection is not None:
        print("----- Iniciando a seleção de características -----")
        save_log("----- Iniciando a seleção de características -----", log_file)
            ...
            if feature_selection_models > 1:
                ...
                if isinstance(models, list):
                    if use_threading:
                        ...
                        for model_name in models:
                            print('iniciando o modelo ', model_name)
                            save_log(f'iniciando o modelo {model_name}', log_file)
                            if model_name == 'lightGBM':
                                model = model_classes[model_name](verbosity=-1)
                            else:
                                model = model_classes[model_name]()
                        
                            lista_de_features[model_name] = selecao_caracteristicas_sfs(X_train, y_train, model, scoring=scoring, cv=cv, n_jobs=n_jobs)
                            print('finalizando o modelo ', model_name)
                            save_log(f'finalizando o modelo {model_name}', log_file)
                            print("\n" * 20)
                ...
                print("Random_forest")
                save_log("Random_forest", log_file)
                model = RandomForestClassifier()
                features = selecao_caracteristicas_sfs(X_train, y_train, model, cv, n_jobs=n_jobs)
                if isinstance(models, list):
                    for model_name in models: 
        ...
        save_log("----- Finalizando a seleção de características -----", log_file)
        print("\n" * 20)
        print()
...
def train(X_train, y_train, lista_de_features, models, opt_metric, opt_hyperparam, levels, n_jobs, cv, log_file):
    ...
    for model_name in models:
        ...
        if opt_hyperparam is not None:
            ...
            if 'Grid-search' == opt_hyperparam:
                ...
                save_log(f'Finalizando o Grid-search {model_name}', log_file)
                print("\n" * 20)
            ...
            save_log("----- Finalizando a Optimização -----", log_file)
            print("\n" * 20)
        ...
        save_log("----- Iniciando o treinamento -----", log_file)

        model.fit(X_train, y_train)
        predicts = model.predict_proba(X_train)

        print("----- Finalizando o treinamento -----")
        save_log("----- Finalizando o treinamento -----", log_file)
        print("\n" * 20)
...
def test(X_test, y_test, lista_de_features, trained_models, metrics, ensemble, log_file):
    results = {}

    X_test_org = X_test.copy()

    print("----- Iniciando os testes -----")
    save_log("----- Iniciando os testes -----", log_file)
    predictions = []
    ...
    save_log("----- Finalizando os testes -----", log_file)
    print("\n" * 20)
